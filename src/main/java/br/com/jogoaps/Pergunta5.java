...
public class Pergunta5 extends JFrame implements ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3512835460232872542L;
	...
	public Pergunta5(double pontuacao, String nomeJogador) {
		...
		setTitle("5º PERGUNTA");
		setSize(900, 600);
		setResizable(false);
		setLocation(50, 50);
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		ImageIcon icon = new ImageIcon(getClass().getClassLoader().getResource("f.jpg"));
		imagem = new JLabel(icon);
		imagem.setLocation(0, 0);
		imagem.setSize(900, 600);

		pergunta = new JLabel(
				"<html>5 - O que causa, com maior frequência,<br> a contaminação do lençol freático ?</html>");
		...
		respostaA = new JButton("A)  Poluição no ar ");
		...
		respostaB = new JButton("B)  Poluição sonora ");
		...
		respostaC = new JButton("C)  Poluição do solo ");// verdade
		...
		respostaD = new JButton("D)  Poluição dos automóveis   ");
