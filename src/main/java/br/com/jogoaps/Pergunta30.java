...
public class Pergunta30 extends JFrame implements ActionListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = 7184332708350767237L;
	...
	public Pergunta30(double pontuacao, String nomeJogador) {
		this.pontuacao = pontuacao;
		this.nomeJogador = nomeJogador;
		setTitle("30º PERGUNTA");
		setSize(900, 600);
		setResizable(false);
		setLocation(50, 50);
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		ImageIcon icon = new ImageIcon(getClass().getClassLoader().getResource("h1.jpg"));
		imagem = new JLabel(icon);
		imagem.setLocation(0, 0);
		imagem.setSize(900, 600);

		pergunta = new JLabel(
				"<html>30 - é o resultado de qualquer tipo de ação ou obra humana capaz de provocar<br>"
				+ "danos no meio ambiente. Esse trecho trata de qual assunto?</html>");
		...
		respostaA = new JButton("A)  Efeito estufa");
		...
		respostaB = new JButton("B)  Reaproveitamento");
		...
		respostaC = new JButton("C)  Reciclagem");
		...
		respostaD = new JButton("D)  Poluição ambiental");// VERDADE
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == respostaA) {
			...
			Pergunta31 obj32 = new Pergunta31(pontuacao, nomeJogador);
