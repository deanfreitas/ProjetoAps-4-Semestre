...
public class Pergunta31 extends JFrame implements ActionListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = -3681839146470622130L;
	...
	public Pergunta31(double pontuacao, String nomeJogador) {
		this.pontuacao = pontuacao;
		this.nomeJogador = nomeJogador;
		setTitle("31º PERGUNTA");
		setSize(900, 600);
		setResizable(false);
		setLocation(50, 50);
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		ImageIcon icon = new ImageIcon(getClass().getClassLoader().getResource("a2.jpg"));
		imagem = new JLabel(icon);
		imagem.setLocation(0, 0);
		imagem.setSize(900, 600);

		pergunta = new JLabel(
				"<html>31 - Quais interferências humanas que acabam aumentando as chances<br>"
				+ "para que ocorra cheias ?</html>");
		...
		respostaA = new JButton("A)  Agressões à natureza, habitação imprópria");// verdade
		...
		respostaB = new JButton("B)  Andar de carro");
		...
		respostaC = new JButton("C)  Aumento da tecnologia na cidade");
		respostaC.setSize(500, 50);
		respostaC.setLocation(10, 280);
		...
		respostaD = new JButton("D)  Não plantar árvores");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == respostaA) {
			pontuacao += 2.5;
			Pergunta32 obj33 = new Pergunta32(pontuacao, nomeJogador);
